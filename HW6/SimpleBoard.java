...
public class SimpleBoard {
  ...
  public int hashCode() {
    // Replace the following line with your solution.
		BigDecimal bgDecimal = new BigDecimal(0);

	  for (int i=0;i<DIMENSION;i++){
		  for (int j=0;j<DIMENSION;j++){
				BigDecimal bgDecima2 = new BigDecimal(this.grid[i][j]*(double)(Math.pow((double) 3, (double) (i*DIMENSION+j))));
				bgDecimal=bgDecimal.add(bgDecima2);
		  }
	  }
	  		// Convert BigDecimal to String
			String s1 = bgDecimal.toString();
			// Take the last 18 digit of the String to fit in a long data type
			if (s1.length()>=18){
				s1=s1.substring(s1.length()-18, s1.length());
			}
			// Convert the chopped string into long type.
			long LongHashCode=Long.parseLong(s1);

			int HashCode=(int)(((long)(LongHashCode))%2147483647);
			return HashCode;
  }
