...
public class UDGraph {
	...
	public UDGraph length2Paths() {
		...
		for (int i = 0; i < vertices; i++) {
			int[] originArray = new int[vertices];
			int[] destArray = new int[vertices];
			...
			for (int j = 0; j < vertices; j++) {
				...
				// } else {
				if (hasEdge(j, i)) {
					originArray[originArrayIndex] = j;
					originArrayIndex++;
					for (int k = destArrayIndex - 1; k >= 0; k--) {
						newGraph.addEdge(j, destArray[k]);
					}
				}
				if (hasEdge(i, j)) {
					destArray[destArrayIndex] = j;
					destArrayIndex++;
					for (int k = originArrayIndex - 1; k >= 0; k--) {
						newGraph.addEdge(originArray[k], j);
					}
				}
	...
	public UDGraph paths(int length) {
		...
		if (length > 1) {
			UDGraph prevGraph = paths(length - 1);
			if (prevGraph.edges > 0) {
				for (int i = 0; i < vertices; i++) {
					int[] originArray = new int[vertices];
					int[] destArray = new int[vertices];
					int originArrayIndex = 0;
