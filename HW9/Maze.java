...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    // Create a DisjointSets data structure for cells 
    int numberCells = horiz*vert;
    DisjointSets cell = new DisjointSets (numberCells);
        
    int totalNoWalls = horiz*(vert-1)+(horiz-1)*vert;
    int arrayWalls[] = new int[totalNoWalls];
    for (int l=0;l<totalNoWalls;l++){
    	arrayWalls[l]=l;
    }
    int index;
    int tmp;
    for (int k=totalNoWalls;k>1 ; k--){
    	index = randInt(k);
    	tmp = arrayWalls[index];
    	arrayWalls[index]=arrayWalls[k-1];
    	arrayWalls[k-1]=tmp;
    	
    }
    for (int wallIndex=0;wallIndex<totalNoWalls;wallIndex++){
    	int root1;
    	int root2;
    	if (arrayWalls[wallIndex]<=(horiz*(vert-1)-1)&&arrayWalls[wallIndex]>=0){
    		int yHorizWallIndex=(int) (arrayWalls[wallIndex]/horiz);
    		int xHorizWallIndex=arrayWalls[wallIndex]-yHorizWallIndex*horiz;
    		root1=cell.find(yHorizWallIndex*horiz+xHorizWallIndex);
    		root2=cell.find((yHorizWallIndex+1)*horiz+xHorizWallIndex);
    		
    		if(root1!=root2){
    			hWalls[xHorizWallIndex][yHorizWallIndex]=false;
    			cell.union(root1, root2);
    		}
    	}
    	if (arrayWalls[wallIndex]>=horiz*(vert-1)&&arrayWalls[wallIndex]<totalNoWalls){
    		int newVertWall = arrayWalls[wallIndex]-horiz*(vert-1);
    		
    		int xVertWallIndex=(int) (newVertWall/vert);	
    		int yVertWallIndex=newVertWall-xVertWallIndex*vert;
    		root1=cell.find(yVertWallIndex*horiz+xVertWallIndex);
    		root2=cell.find((yVertWallIndex)*horiz+xVertWallIndex+1);
    		if(root1!=root2){
    			vWalls[xVertWallIndex][yVertWallIndex]=false;
    			cell.union(root1, root2);
    		}
    	}
    }
